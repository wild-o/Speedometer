...
function rotateCar(car, degrees, d) {
  ...
  if (((currentAngle + degrees) * (d * car.turningSpeed)) > 360) {
    car.angle_in_degrees = (((currentAngle + degrees) * (d * car.turningSpeed)) - 360)
  }
  else if (((currentAngle + degrees) * (d * car.turningSpeed)) < 0) {
    car.angle_in_degrees = (((currentAngle + degrees) * (d * car.turningSpeed)) + 360)
  }
  else {
    car.angle_in_degrees = ((currentAngle + degrees) * (d * car.turningSpeed))
  }
...
function Accelerate(car) {
  ...
  if (shiftCount !== 0 && throttle) {
    car.velocity_x += LINEAR_ACC * delta
  }
  // else if (shiftCount === -1 && throttle) {
  //   car.velocity_x -= LINEAR_ACC * delta
  // }
}
...
function shift(value) {
  ...
  */
  ...
  const REVERSE = -1
  const NEUTRAL = 0
  if (value === 1 && shiftCount < 6 && !(shiftCount === REVERSE && car.velocity_x > 0)) {
    upperBound += 25;
    shiftCount++;
  }
  else if (value === -1 && shiftCount > REVERSE && !(shiftCount === 0 && car.velocity_x > 0)) {
    upperBound -= 25;
    shiftCount--;
...
function updateCarPosition(car, delta) {
  ...
  else if (isBrake && car.velocity_x > 0) {
    car.velocity_x -= (BRAKE_FORCE * delta)
  }
  else if (!throttle) {
    Decelerate(car) // might need to use delta here
  }
  newNum.innerHTML = Math.round(car.velocity_x) // using round to smooth out the display
